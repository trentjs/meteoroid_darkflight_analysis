...
class OrbitObject(object):
    ...
    def tisserand_criterion_wrt_jupiter(self):
        ...
        '''
        T_j = (SMA_JUPITER / self.semi_major_axis +
               2 * np.cos(self.inclination) *
               np.sqrt(self.semi_major_axis / SMA_JUPITER * (1 - self.eccentricity**2)))
        return T_j
...
def generate_ephemeris(pos_hci, t_jd):
    ...
    ephem_dict['phase_angle'] = np.rad2deg(np.arccos(np.sum(pos_hci * pos_eci, axis=0)
        / (norm(pos_hci, axis=0) * norm(pos_eci, axis=0))))

    # Calculate elongation angle
    pos_sun = pos_eci - pos_hci
    ...
    ephem_dict['distance'] = norm(pos_eci, axis=0) / 1000 #km
